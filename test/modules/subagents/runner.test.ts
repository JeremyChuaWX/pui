...
import { runSubagent } from "#modules/subagents/runner.ts";
...
function startingJob(id = "outer-id"): SubagentJobV1 {
    const initial = createInitialSubagentJob({
        id,
        agent: "explore",
        model: "fixture/model",
        cwd,
...
describe("runSubagent", () => {
    test("streams fragmented JSONL, tracks parallel tools, and aggregates each assistant once", async () => {
        ...
        expect(result.job.model).toBe("fixture/model");
        expect(result.job.usage).toEqual({
            input: 30,
            output: 6,
            cacheRead: 3,
            cacheWrite: 1,
        ...
        expect(result.job.activeTools).toEqual([]);
    ...
    test("bounds snapshots while retaining omitted tools through their end events", async () => {
        ...
        expect(snapshots.every(isSubagentJobV1)).toBe(true);
    ...
    test("keeps the long model label through the final terminal snapshot", async () => {
        const job = startingJob();
        job.model = "fixture/model:off";
        const { result, snapshots } = await runFixture("success", { job });

        expect(snapshots.at(-1)?.status).toBe("succeeded");
        expect(snapshots.at(-1)?.model).toBe("fixture/model:off");
        expect(result.job.model).toBe("fixture/model:off");
    });
    ...
    test("keeps malformed output as a bounded diagnostic without crashing", async () => {
        ...
        expect(
            result.job.recentActivity.some((item) => item.kind === "diagnostic" && item.title.includes("malformed")),
        ).toBe(true);
    });
    ...
    test("fails actionably when no final assistant response is emitted", async () => {
        ...
        expect(result.job.error).toContain("without a final assistant response");
    });
    ...
    test("rejects an invalid finalized assistant message without crashing", async () => {
        ...
        expect(result.job.status).toBe("failed");
        expect(result.job.error).toContain("without a final assistant response");
        expect(result.job.recentActivity.some((item) => item.title.includes("invalid finalized"))).toBe(true);
    });

    test("includes bounded stderr in nonzero-exit diagnostics", async () => {
        const { result } = await runFixture("failure");
        expect(result.job.status).toBe("failed");
        expect(result.job.error).toContain("exit code 7");
        expect(result.job.error).toContain("fixture child failed actionably");
    });
    ...
    test("distinguishes timeout from user cancellation and force-kills stubborn children", async () => {
        ...
        expect(timedOut.result.job.status).toBe("timed_out");
        expect(timedOut.result.job.error).toContain("wall clock");

        const controller = new AbortController();
        setTimeout(() => controller.abort(), 20);
        const cancelled = await runFixture("hang", { limits: wallClock(5_000), signal: controller.signal });
        expect(cancelled.result.job.status).toBe("cancelled");
        expect(cancelled.result.job.error).toContain("cancelled");
    });
    ...
    test("returns structured failure for a spawn error", async () => {
        ...
        expect(result.job.status).toBe("failed");
        expect(result.job.error).toContain("Unable to start child Pi");
    });
});
