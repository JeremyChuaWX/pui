...
import { resolveWorkingDirectory } from "#modules/subagents/working-directory.ts";
...
describe("resolveWorkingDirectory", () => {
    ...
    test("rejects an empty, missing, or non-directory cwd with a specific message", async () => {
        ...
        await expect(resolveWorkingDirectory("working-directory.test.ts", parent)).rejects.toThrow(
            "is not a directory",
        );
    });
});
