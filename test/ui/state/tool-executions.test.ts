...
} from "#ui/state/tool-executions.js";
...
describe("tool execution reducer", () => {
    ...
    test("retains an ended execution until its persisted tool result is visible", () => {
        let state: ToolExecutionState = new Map();
        state = reduce(state, { type: "tool_execution_start", toolCallId: "one", toolName: "read", args: {} }, 10);
        state = reduce(
            state,
            {
                type: "tool_execution_end",
    ...
    test("uses pending calls as a safety check and only removes stale state on settlement", () => {
        let state: ToolExecutionState = new Map();
        state = reduce(state, { type: "tool_execution_start", toolCallId: "live", toolName: "read", args: {} }, 10);
        state = reduce(state, { type: "tool_execution_start", toolCallId: "stale", toolName: "bash", args: {} }, 11);
