...
import { checkBoundaries, collectImportEdges, type ImportEdge } from "../../scripts/check-boundaries.ts";
...
describe("clean graph", () => {
    test("every sanctioned edge produces no violations", () => {
        const edges: ImportEdge[] = [
            ...
            { from: "modules/subagents/manager.ts", to: "modules/subagents/protocol.ts" },
            { from: "modules/subagents/manager.ts", to: "shared/lib/clock.ts" },
            { from: "modules/file-search/process.ts", to: "modules/file-search/bounded-process.ts" },
            // shared -> shared
            { from: "shared/lib/validate.ts", to: "shared/lib/runtime.ts" },
        ];
...
describe("forbidden edges", () => {
    ...
    test("production code -> files outside src is a violation", () => {
...
describe("source test placement", () => {
    test("a test file under src is a violation", () => {
        const edges: ImportEdge[] = [{ from: "modules/web/tool-shell.test.ts", to: "modules/web/tool-shell.ts" }];
        expect(checkBoundaries(edges)[0]?.rule).toContain("test/");
    });

    test("production code must not import a test file", () => {
