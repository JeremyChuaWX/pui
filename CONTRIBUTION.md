...
- follow the layer rules in `docs/ARCHITECTURE.md`: five layers (`app/`, `ui/`, `pi-core/`,
  `modules/`, `shared/`) with one-way dependency edges, enforced by
  `scripts/check-boundaries.ts` inside the check gate
- a feature lives in one Module under `modules/<name>/`; from outside a Module, import only its
  Interfaces Directory (`interfaces/pi.ts`, `interfaces/host.ts`, `interfaces/ui.ts`,
  `interfaces/api.ts`). Modules never import each other; cross-cutting code goes in `shared/`
