...
For automation, `pui workflow [--cwd <project>] <path> [JSON args]` runs a file without opening the TUI or creating a Pi session and prints its JSON result. Supplying the path on this command line explicitly authorizes the file for that invocation, bypassing UI approval and project-trust prompts. Runtime sandboxing, host policy, durable storage, external-Node resolution, and worktree isolation are unchanged; failures return a nonzero exit.
