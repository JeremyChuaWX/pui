...
pui registers the bundled `workflow` tool by default. Exact workflow scripts are shown in an inline transcript approval block; use **PageUp**/**PageDown** to inspect long scripts, then choose **Run once** or **Trust unchanged script in this project**. Scripts never run when confirmation is unavailable or denied. Durable run artifacts live outside projects under `~/.pi/agent/workflow-runs/<project-hash>/<run-id>/`; injected `WorkflowRunStorage` roots keep recovery tests hermetic. Launch bytes and policy are immutable, snapshots are atomic, completion journals are append-only and fsynced, and corrupt/truncated or symlinked artifacts fail closed.
