...
const budgetLaunch = (backend: WorkflowBackend, script: string, limits: Record<string, number> = {}) =>
...
describe("workflow budgets and pause", () => {
    test("caps concurrency and pause gates queued agents until resume", async () => {
        ...
        const backend = createBackend({ agentExecutor: executor });
        const { runId } = await budgetLaunch(
            backend,
    ...
    test("enforces the agent cap and warns at 25 scheduled agents", async () => {
        const backend = createBackend({ agentExecutor: async ({ prompt }) => ({ value: prompt }) });
        const capped = await budgetLaunch(backend, `for(let i=0;i<3;i++) await agent(String(i),{role:"explore"})`, {
            maxAgents: 2,
        });
        ...
        const large = await budgetLaunch(backend, `for(let i=0;i<26;i++) await agent(String(i),{role:"explore"})`, {
            maxAgents: 26,
        });
    ...
    test("bounds retries and enforces agent and run timeouts", async () => {
        let calls = 0;
        const retrying = createBackend({
            agentExecutor: async () => {
                calls++;
                throw new Error("retry me");
            },
        });
        const retried = await budgetLaunch(retrying, `await agent("x",{role:"explore",retries:99})`);
        await waitFor(() => retrying.inspect(retried.runId).run.status === "failed");
        expect(calls).toBe(4);
        valid(retrying, retried.runId);
        await retrying.shutdown();

        const timed = createBackend({ agentExecutor: async () => await new Promise(() => {}) });
        const agentTimeout = await budgetLaunch(timed, `await agent("x",{role:"explore",timeoutMs:20})`);
        ...
        const runTimeout = await budgetLaunch(total, `await agent("x",{role:"explore",timeoutMs:1000})`);
    ...
    test("retains consumed usage when token and cost budgets are exceeded", async () => {
        for (const [limits, usage, message] of [
            ...
            const backend = createBackend({ agentExecutor: async () => ({ value: "done", usage }) });
            const { runId } = await budgetLaunch(backend, `await agent("x",{role:"explore"})`, limits);
            await waitFor(() => backend.inspect(runId).run.status === "failed");
