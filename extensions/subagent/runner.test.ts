...
function startingDetails(id = "outer-id"): SubagentDetailsV1 {
    const initial = createInitialSubagentDetails({
        id,
        agent: "explore",
        model: "fixture/model",
        cwd,
...
describe("runSubagent", () => {
    test("streams fragmented JSONL, tracks parallel tools, and aggregates each assistant once", async () => {
        ...
        expect(
            snapshots.some((item) => item.run.activeTools.length === 1 && item.run.activeTools[0]?.id === "tool-a"),
        ).toBe(true);
    ...
    test("keeps malformed output as a bounded diagnostic without crashing", async () => {
        ...
        expect(
            result.details.run.recentActivity.some(
                (item) => item.kind === "diagnostic" && item.title.includes("malformed"),
            ),
        ).toBe(true);
    });
