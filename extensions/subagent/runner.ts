...
export type SpawnChild = (command: string, args: readonly string[], options: {
  cwd: string;
  shell: false;
  detached: boolean;
...
export async function runSubagent(options: RunSubagentOptions): Promise<SubagentRunResult> {
  ...
  const processEvent = (eventValue: unknown) => {
    ...
    if (event.type === "turn_start") {
      ...
      addActivity("turn", index === undefined ? "Turn started" : `Turn ${index + 1} started`, undefined, timestamp);
    ...
    if (event.type === "turn_end") {
      const index = numberOrUndefined(event.turnIndex);
      addActivity("turn", index === undefined ? "Turn completed" : `Turn ${index + 1} completed`, undefined, timestamp);
      details = updateSubagentDetails(details, { phase: activeTools.size > 0 ? "tool" : "thinking" }, timestamp);
      emit();
      return;
    }
  ...
  const finalize = (): SubagentRunResult => {
    ...
    if (terminationReason === "cancelled") {
      ...
      status = "timed_out";
      error = boundedDiagnostic(`Subagent timed out after ${options.timeoutMs / 1000} seconds.`, outputOrDiagnostic);
    } else if (spawnError) {
      status = "failed";
      ...
      status = "failed";
      error = boundedDiagnostic("Subagent exited without a final assistant response.", stderrOrDiagnostic || diagnostics);
    } else if (stopReason === "aborted") {
      status = "failed";
      error = boundedDiagnostic(finalMessage.errorMessage || "Child assistant stopped unexpectedly.", outputOrDiagnostic);
    } else if (stopReason === "error") {
      status = "failed";
      error = boundedDiagnostic(finalMessage.errorMessage || "Subagent model request failed.", outputOrDiagnostic);
    } else {
      status = "succeeded";
    }
  ...
  return new Promise<SubagentRunResult>((resolve) => {
    child!.stdout.on("data", (chunk: Buffer | string) => {
      try {
        ...
        addDiagnostic(`Unable to process child output: ${error instanceof Error ? error.message : String(error)}`);
      }
    });
    child!.stdout.on("error", (error) => addDiagnostic(`Child stdout failed: ${error.message}`));
