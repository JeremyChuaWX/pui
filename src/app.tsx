...
export function App(props: { controller: PuiController }) {
    ...
    function openCommands(): void {
        ...
        setDialog({
            ...
            items: [
                ...
                command(
                    "Edit in nvim",
                    "Edit the prompt with the last agent response as reference",
    ...
    useKeyboard((key) => {
        ...
        if (completions) {
            ...
            const confirm =
                key.name === "tab" || (!key.shift && !key.ctrl && !key.meta && !key.option && isEnterName(key.name));
            if (confirm) {
                key.preventDefault();
                key.stopPropagation();
