...
Reload, session replacement, fork, and quit abort all queued/running background jobs. Shutdown waits concurrently for settlement with a bounded teardown, removes full-output directories created by this extension, emits `reset`, and never sends a late result message or writes a late spill. A Job that settles during shutdown is marked consumed and dropped. Jobs are intentionally not restored or reattached.
